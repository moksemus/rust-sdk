...
use super::types::{
    ...
    ComponentListResponse, DocumentationTopicsResponse, ComponentTagsResponse,
};
...
impl ReactComponentServer {
    ...
    async fn list_documentation_topics(&self) -> Result<Json<DocumentationTopicsResponse>, McpError> {
        let mut topics: Vec<String> = self.documentation.keys().cloned().collect();
        topics.sort();
        Ok(Json(DocumentationTopicsResponse { topics }))
    }
    ...
    async fn get_component_tags(&self) -> Result<Json<ComponentTagsResponse>, McpError> {
        ...
        sorted_tags.sort();
        Ok(Json(ComponentTagsResponse { tags: sorted_tags }))
    }
}
